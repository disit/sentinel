    ...
    if "False" == os.getenv("running-as-kubernetes","False"):
        ...
        for raw_json in raw_jsons:
            for item in raw_json["items"]:
                ...
                except: # maybe it is nowhere?
                    conversion["Node"] = "No node"
                try:
...
def create_app():
    ...
    def get_top_single():  #not used anymore
        if 'username' in session:
            ...
            try:
                ...
                with mysql.connector.connect(**db_conn_info) as conn:
                    ...
                    if request.form.to_dict()['kind'] == "Kubernetes":
                        ...
                        cursor.execute(query, (request.form.to_dict()['id'],request.form.to_dict()['category'],request.form.to_dict()['contacts'],request.form.to_dict()['position'],request.form.to_dict()['kind'],request.form.to_dict()['severity'],))
        ...
        if 'username' in session:
            try:
                ...
                with mysql.connector.connect(**db_conn_info) as conn:
                    cursor = conn.cursor(buffered=True)
                    if request.form.to_dict()['where_to_run'] != "":
                        query = '''INSERT INTO `checker`.`cronjobs` (`name`, `command`, `category`, `where_to_run`, `disabled`, `restart_logic`, `description`, `timeout_time`, `retries`, `retries_wait`, `ip`, `target`, `contacts`, `severity`) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);'''
                        cursor.execute(query, (request.form.to_dict()['name'],request.form.to_dict()['command'],request.form.to_dict()['category'],
                                               ...
                                               request.form.to_dict()['IPAdd'],request.form.to_dict()['TargetAdd'],request.form.to_dict()['contacts'],request.form.to_dict()['severity'],))
                    else:
                        query = '''INSERT INTO `checker`.`cronjobs` (`name`, `command`, `category`, `where_to_run`, `disabled`, `restart_logic`, `description`, `timeout_time`, `retries`, `retries_wait`, `ip`, `target`, `contacts`, `severity`) VALUES (%s, %s, %s, NULL, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);'''
                        cursor.execute(query, (request.form.to_dict()['name'],request.form.to_dict()['command'],request.form.to_dict()['category'],
    ...
    @app.route("/delete_extra_resource", methods=['POST', 'DELETE'])
    ...
    @app.route("/delete_test", methods=['POST', 'DELETE'])
    def delete_test():
    ...
    @app.route("/delete_complex_test", methods=['POST', 'DELETE'])
    ...
    @app.route("/delete_category", methods=['POST', 'DELETE'])
    def delete_category():
    ...
    @app.route("/get_complex_test_buttons", methods=["GET"])
...
SELECT datetime,result,errors,name,command,categories.category,cronjobs.idcronjobs,cronjobs.disabled,description,restart_logic FROM RankedEntries join cronjobs on cronjobs.idcronjobs=RankedEntries.id_cronjob join categories on categories.idcategories=cronjobs.category WHERE row_num = 1;'''
        ...
        for raw_json in raw_jsons:
            for item in raw_json["items"]:
                try:
                    ...
                    print_debug_log(f"Failed to properly read a pod: {traceback.format_exc()}\nThis pod will not be included in the response")
        if do_not_jsonify:
            return conversions
    ...
    async def generate_pdf(): # TODO need to add snmp tops
...
SELECT datetime,result,errors,name,command,categories.category FROM RankedEntries join cronjobs on cronjobs.idcronjobs=RankedEntries.id_cronjob join categories on categories.idcategories=cronjobs.category WHERE row_num = 1;'''
    ...
    @app.route("/delete_saved_host", methods=['POST', 'DELETE'])
    def delete_host():
        if 'username' in session:
            if session['username']!="admin":
                return render_template("error_showing.html", r = "You do not have the privileges to access this webpage."), 401
    ...
    @app.route("/delete_sentinel_host", methods=['POST', 'DELETE'])
    ...
    @app.route("/delete_snmp_host", methods=['POST', 'DELETE'])
    ...
    @app.route("/delete_configuration_retrieval", methods=['POST', 'DELETE'])
